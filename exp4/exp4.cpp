...
// 计算最大矩形面积 - 使用数组实现栈来避免MySTL vector的问题
...
// 生成随机高度数组 - 使用更小的范围 to avoid memory issues
vector<int> generateRandomHeights() {
    int length = rand() % 100 + 1; // Reduced from 100000 to 100
    vector<int> heights(length, length); // Create vector with size=capacity=length, initialized to 0
